...
Analyze this transcript and find the most compelling moments that would make great short-form clips (15-90 seconds each). Look for:
...
- Find 3-10 highlights depending on content density
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        for (const h of parsed) {
          const chunkSegTexts = segments
            .filter((s) => s.start >= h.startTime && s.end <= h.endTime)
            .map((s) => s.text)
            .join(" ");
          ...
          allHighlights.push({
            ...
            endTime: h.endTime,
            score: Math.min(10, Math.max(1, h.score || 5)),
            transcript: chunkSegTexts || h.title,
          });
        }
      }
    } catch (err) {
